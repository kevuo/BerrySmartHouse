...
//This is intended as an wrapper for the gpio dynamic library
//to be used in nodejs, as a server in the tiny but powerfull raspberry pi
//The way its designed allows to write to raspberry's gpio pins, and provides
...
//Set all lights to the given state
int setAllLights( char * valueChar){
	int pinValue= strtol(valueChar, NULL, 10);
	for(int i = 0; i<6; i++){
		digitalWrite(ledGPIOArray[i], pinValue);
	}
  	return 0;
}
...
int doorsState(){
	...
	json_object_object_add(jsonObjTop,"doors",jsonArray);
	//its better to return a string rather than open and write a file
	//FILE *fp = fopen("../data/lightsState.json", "w");
	printf("%s", json_object_to_json_string(jsonObjTop));
	return 0;
...
void help(){
	...
	printf("%s\n", "'-l value' to set all lights to the given state");
